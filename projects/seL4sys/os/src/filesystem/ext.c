...
int readGroupHeader (SuperBlock *superBlock, GroupDesc *groupDesc) {
	...
	read_disk((void*)superBlock, sizeof(SuperBlock), 1, 0);
	...
	read_disk((void*)groupDesc, sizeof(GroupDesc), groupNum, sizeof(SuperBlock));
	return 0;
}
...
int readBlock (SuperBlock *superBlock, Inode *inode, int blockIndex, uint8_t *buffer) {
	...
	if (blockIndex < bound0) {
		...
		read_disk((void*)buffer, sizeof(uint8_t), superBlock->blockSize, inode->pointer[blockIndex] * SECTOR_SIZE);
		return 0;
	}
		...
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->singlyPointer * SECTOR_SIZE);
		//fseek(file, singlyPointerBuffer[blockIndex - bound0] * SECTOR_SIZE, SEEK_SET);
		//fread((void*)buffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)buffer, sizeof(uint8_t), superBlock->blockSize, singlyPointerBuffer[blockIndex - bound0] * SECTOR_SIZE);
		return 0;
	}
		...
		read_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->doublyPointer * SECTOR_SIZE);
		...
		//fread((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[quotient] * SECTOR_SIZE);
		//fseek(file, singlyPointerBuffer[remainder] * SECTOR_SIZE, SEEK_SET);
		//fread((void*)buffer, sizeof(uint8_t) , superBlock->blockSize, file);
		read_disk((void*)buffer, sizeof(uint8_t), superBlock->blockSize, singlyPointerBuffer[remainder] * SECTOR_SIZE);
		return 0;
	}
		...
		read_disk((void*)triplyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->triplyPointer * SECTOR_SIZE);
		...
		//fread((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, triplyPointerBuffer[quotient] * SECTOR_SIZE);
...
int writeBlock (SuperBlock *superBlock, Inode *inode, int blockIndex, uint8_t *buffer) {
	...
	if (blockIndex < bound0) {
		...
		write_disk((void*)buffer, sizeof(uint8_t), superBlock->blockSize, inode->pointer[blockIndex] * SECTOR_SIZE);
		return 0;
	}
	else if (blockIndex < bound1) {
		//fseek(file, inode->singlyPointer * SECTOR_SIZE, SEEK_SET);
		...
		//fseek(file, singlyPointerBuffer[blockIndex - bound0] * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)buffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)buffer, sizeof(uint8_t), superBlock->blockSize, singlyPointerBuffer[blockIndex - bound0] * SECTOR_SIZE);
		return 0;
	}
	else if (blockIndex < bound2) {
		//fseek(file, inode->doublyPointer * SECTOR_SIZE, SEEK_SET);
		...
		//fseek(file, singlyPointerBuffer[remainder] * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)buffer, sizeof(uint8_t) , superBlock->blockSize, file);
		write_disk((void*)buffer, sizeof(uint8_t) , superBlock->blockSize, singlyPointerBuffer[remainder] * SECTOR_SIZE);
		return 0;
	}
	else if (blockIndex < bound3) {
		//fseek(file, inode->triplyPointer * SECTOR_SIZE, SEEK_SET);
...
int getAvailBlock (SuperBlock *superBlock, GroupDesc *groupDesc, int *blockOffset) {
	...
	read_disk((void*)&blockBitmap, sizeof(BlockBitmap), 1, blockBitmapOffset * SECTOR_SIZE);
...
int allocLastBlock (SuperBlock *superBlock, GroupDesc *groupDesc, Inode *inode, int inodeOffset, int blockOffset) {
	...
	if (inode->blockCount < bound0) {
		...
		write_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, singlyPointerBufferOffset * SECTOR_SIZE);
	}
		...
		//fseek(file, inode->singlyPointer * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->singlyPointer * SECTOR_SIZE);
	}
	else if (inode->blockCount == bound1) {
		//getAvailBlock(file, superBlock, groupDesc, &singlyPointerBufferOffset);
		getAvailBlock(superBlock, groupDesc, &singlyPointerBufferOffset);
		...
		write_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBufferOffset * SECTOR_SIZE);
		...
		//fseek(file, inode->doublyPointer * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->doublyPointer * SECTOR_SIZE);
	}
		...
		//fread((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[(inode->blockCount - bound1) / divider0] * SECTOR_SIZE);
		singlyPointerBuffer[(inode->blockCount - bound1) % divider0] = blockOffset;
		// write back
		//fseek(file, doublyPointerBuffer[(inode->blockCount - bound1) / divider0] * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[(inode->blockCount - bound1) / divider0] * SECTOR_SIZE);
	}
	else if (inode->blockCount == bound2) {
		//getAvailBlock(file, superBlock, groupDesc, &singlyPointerBufferOffset);
		getAvailBlock(superBlock, groupDesc, &singlyPointerBufferOffset);
		...
		write_disk((void*)triplyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, triplyPointerBufferOffset * SECTOR_SIZE);
	}
		...
		//fseek(file, inode->triplyPointer * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)triplyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)triplyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, inode->triplyPointer * SECTOR_SIZE);
	}
		...
		//fread((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, triplyPointerBuffer[(inode->blockCount - bound2) / divider1] * SECTOR_SIZE);
		...
		//fseek(file, triplyPointerBuffer[(inode->blockCount - bound2) / divider1] * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, triplyPointerBuffer[(inode->blockCount - bound2) / divider1] * SECTOR_SIZE);
	}
		...
		//fread((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[(inode->blockCount - bound2) % divider1 / divider0] * SECTOR_SIZE);
		singlyPointerBuffer[(inode->blockCount - bound2) % divider1 % divider0] = blockOffset;
		// write back
		//fseek(file, doublyPointerBuffer[(inode->blockCount - bound2) % divider1 / divider0] * SECTOR_SIZE, SEEK_SET);
		//fwrite((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		write_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[(inode->blockCount - bound2) % divider1 / divider0] * SECTOR_SIZE);
	}
	else
		return -1;
	// write back inode
...
int freeLastBlock (SuperBlock *superBlock, GroupDesc *groupDesc, Inode *inode, int inodeOffset) {
	...
	if (inode->blockCount < bound0) {
		...
		//fread((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[0] * SECTOR_SIZE);
		//setAllocBlock(file, superBlock, groupDesc, singlyPointerBuffer[0]);
		setAllocBlock(superBlock, groupDesc, singlyPointerBuffer[0]);
		...
		//fread((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)doublyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, triplyPointerBuffer[0] * SECTOR_SIZE);
		//fseek(file, doublyPointerBuffer[0] * SECTOR_SIZE, SEEK_SET);
		//fread((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
		read_disk((void*)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, doublyPointerBuffer[0] * SECTOR_SIZE);
		//setAllocBlock(file, superBlock, groupDesc, singlyPointerBuffer[0]);
...
int getDirEntry (SuperBlock *superBlock, Inode *inode, int dirIndex, DirEntry *destDirEntry) {
	...
	if (i == inode->blockCount)
		...
		strncpy(destDirEntry->name, dirEntry[j].name, NAME_LENGTH);
		return 0;
	}
}
...
int readInode (SuperBlock *superBlock, GroupDesc *groupDesc,
	...
	int j = 0;
	int ret = 0;
	int cond = 0; // label to mark if destFilePath is ended w	ith '/'
	*inodeOffset = 0;
	uint8_t buffer[superBlock->blockSize];
	DirEntry *dirEntry = NULL;
	...
	*inodeOffset = groupDesc->inodeTable * SECTOR_SIZE; // inodeOffset of '/'
	...
	read_disk((void*)destInode, sizeof(Inode), 1, *inodeOffset);
		...
		for (i = 0; i < blockCount; i ++) {
			...
			for (j = 0; j < superBlock->blockSize / sizeof(DirEntry); j ++) {
				if (dirEntry[j].inode == 0) // empty dirEntry
					continue;
				else if (strncmp(dirEntry[j].name, destFilePath + count, size) == 0) {
...
int getAvailInode (SuperBlock *superBlock, GroupDesc *groupDesc, int *inodeOffset) {
	...
	read_disk((void*)&inodeBitmap, sizeof(InodeBitmap), 1, inodeBitmapOffset * SECTOR_SIZE);
...
int allocInode (SuperBlock *superBlock, GroupDesc *groupDesc,
	...
	DirEntry *dirEntry = NULL;
	uint8_t buffer[superBlock->blockSize];
	int length = strlen(destFilename);
	...
	for (i = 0; i < fatherInode->blockCount; i ++) {
		...
		for (j = 0; j < superBlock->blockSize / sizeof(DirEntry); j ++) {
			if (dirEntry[j].inode == 0) // a valid empty dirEntry
				break;
			else if (strncmp(dirEntry[j].name, destFilename, length) == 0)
	...
	write_disk((void*)fatherInode, sizeof(Inode), 1, fatherInodeOffset);
	/* setting destInode */
	...
	write_disk((void*)destInode, sizeof(Inode), 1, *destInodeOffset);
